...
cosine_sim_credits_key_genre = cosine_similarity(count_matrix, count_matrix)
...
def get_recommendations(title):
    ...
    # similarity scores looking at the plot
    sim_scores = list(enumerate(cosine_sim_plot[idx]))
    sim_scores = sorted(sim_scores, key=lambda x: x[1], reverse=True)
    sim_scores = sim_scores[1:26]

    # similarity scores looking at the credits, keywords, genres
    sim_scores_2 = list(enumerate(cosine_sim_credits_key_genre[idx]))
    sim_scores_2 = sorted(sim_scores_2, key=lambda x: x[1], reverse=True)
    sim_scores_2 = sim_scores_2[1:26]
    sim_scores.extend(sim_scores_2)
    sim_scores = sorted(sim_scores, key=lambda x: x[1], reverse=True)
...
def popular_recommendations(title, cosine_sim=cosine_sim_plot):
    idx = indices[title]
